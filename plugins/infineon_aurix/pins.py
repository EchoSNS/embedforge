"""AURIX pin provider — TriCore uses Pxx.y naming (P00.0 to P40.15)."""
from __future__ import annotations
import re
from typing import Dict, List
from plugins.base import PinCapabilityProvider, PinMapping

_AURIX_PIN_RE = re.compile(r"^P(\d{2})\.(\d+)$")

class AurixPinProvider(PinCapabilityProvider):
    def get_available_pins(self, peripheral: str, function: str = "") -> List[PinMapping]:
        try:
            ...
            aurix = [d for d in devices if "TC" in d.get("device", "").upper()]
            if not aurix:
                return []
            device_id = db.find_device(aurix[0]["device"])
    ...
    def validate_pin(self, symbol: str) -> bool:
        m = _AURIX_PIN_RE.match(symbol)
        if not m:
            return False
        port = int(m.group(1))
        pin = int(m.group(2))
        return port <= 40 and pin <= 15

"""Nordic pin provider — nRF52/53 use P0.xx and P1.xx naming."""
...
_NRF_PIN_RE = re.compile(r"^P(\d+)\.(\d+)$")


class NordicPinProvider(PinCapabilityProvider):
    """Pin provider for Nordic nRF MCUs — P0.00-P0.31, P1.00-P1.15."""
    ...
    def get_available_pins(self, peripheral: str, function: str = "") -> List[PinMapping]:
        try:
            ...
            nordic = [d for d in devices if "Nordic" in d.get("vendor", "") or "nRF" in d.get("family", "")]
            if not nordic:
                return []
            device_id = db.find_device(nordic[0]["device"])
    ...
    def validate_pin(self, symbol: str) -> bool:
        m = _NRF_PIN_RE.match(symbol)
        if not m:
            return False
        port = int(m.group(1))
        pin = int(m.group(2))
        return port in (0, 1) and 0 <= pin <= 31
